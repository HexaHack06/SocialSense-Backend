const { getAudienceData } = require('../services/audienceService');
...
const getAudience = async (req, res) => {
  try {
    ...
    console.error('Error in getAudience controller:', error);
    return res.status(500).json({
      success: false,
      message: 'Internal server error while fetching audience data'
    });
  }
};
